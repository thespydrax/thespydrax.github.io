...
const PLAYLISTS = {
  ...
  bangla: {
    label: 'Bangla',
    url:   'https://iamshajon.com/playlist/bangla.json',
  },
};

// Channels pinned to the top of each playlist (case-insensitive name match, order matters)
const PINNED_TOP = {
  fifa: ['Somoy TV', 'Toffee'],
};
...
async function loadPlaylist(key, force = false) {
  ...
  try {
    ...
    if (!parsed.length) throw new Error('Empty or invalid JSON playlist');

    S.channels[key] = applyPinning(key, parsed);
 ...
 *
 * Expects an array of channel objects with fields:
 *   name, logo, group, url            — always present
 *   type ("dash" | absent for HLS)    — optional
 *   kid, key                           — optional ClearKey DRM
...
function parseJSON(data) {
  ...
  return data
    ...
    .map((ch, idx) => ({
      id:    ch.id    || `ch-${idx}`,
      name:  (ch.name  || 'Unknown').trim(),
      logo:  ch.logo  || '',
      group: ch.group || '',
      url:   ch.url.trim(),
      type:  ch.type === 'dash' || ch.url.includes('.mpd') ? 'dash' : 'hls',
      kid:   ch.kid   || '',
      key:   ch.key   || '',
    }));
}
...
/**
 * Move pinned channels to the top of the list for a given playlist key.
 * Pin order matches the order declared in PINNED_TOP.
 */
function applyPinning(key, channels) {
  const pins = (PINNED_TOP[key] || []).map((n) => n.toLowerCase());
  if (!pins.length) return channels;
  ...
  channels.forEach((ch) => {
    ...
    const pinIdx    = pins.findIndex((p) => nameLower.includes(p));
    if (pinIdx !== -1) {
      pinned[pinIdx] = pinned[pinIdx] || [];
      pinned[pinIdx].push(ch);
    } else {
      rest.push(ch);
    }
  });

  // Flatten pinned slots in declared order, then append the rest
  return [...pinned.flat(), ...rest];
}
